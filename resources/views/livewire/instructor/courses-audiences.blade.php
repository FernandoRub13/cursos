            ...
            <div class="card-body bg-gray-100">
                @if ($audience->id == $item->id)
                    <form wire:submit.prevent='update' >
                        <input wire:model="audience.name" class="appearance-none border-2 border-gray-200 rounded p-2 w-full" type="text">
                        @error('audience.name')
    ...
    <article class="card">
        <div class="card-body bg-gray-1">
            <form wire:submit.prevent='store' >
                <input wire:model="name" class="appearance-none border-2 border-gray-200 rounded p-2 w-full" placeholder="Agregar el nombre de la audiencia" >
                @error('name')
                    <span class="text-red-500 text-xs font-bold">{{ $message }}</span>
                @enderror
